...
@section('content')
    <div class="max-w-2xl mx-auto animate-fade-in-up">
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-black text-white">Edit Data User</h1>
            <p class="text-gray-400 text-sm mt-1">Mengubah data: <span class="text-luxury-gold font-bold">{{ $user->name }}</span></p>
        </div>
        ...
        <div class="premium-glass p-8 rounded-[2rem] relative overflow-hidden">
            ...
            <form action="{{ route('users.update', $user->id) }}" method="POST" class="space-y-6 relative z-10">
                ...
                <div class="group">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <label class="cursor-pointer">
                            <input type="radio" name="role" value="staff" class="peer sr-only" {{ $user->role == 'staff' ? 'checked' : '' }}>
                ...
                <div class="border-t border-luxury-gold/10 my-6 relative">
                    <span class="absolute top-[-10px] left-0 bg-[#121212] pr-2 text-xs text-luxury-gold font-bold uppercase tracking-widest">Ganti Password (Opsional)</span>
                </div>

                <div class="p-4 border border-luxury-gold/10 rounded-xl bg-black/20">
                    <p class="text-xs text-gray-500 mb-4">*Kosongkan jika tidak ingin mengubah password.</p>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="group">
                            <label class="label-gold text-[10px]">Password Baru</label>
                            <input type="password" name="password" class="input-gold" placeholder="******">
                        </div>
                        <div class="group">
                            <label class="label-gold text-[10px]">Konfirmasi Password</label>
                            <input type="password" name="password_confirmation" class="input-gold" placeholder="******">
                        </div>
                    </div>
                </div>
